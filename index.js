...
app.post('/webhook',line.middleware(config),(req,res)=>{
  Promise.all(req.body.events.map(handleEvent)).then(()=>res.json({status:'ok'})).catch(err=>{console.error(err);res.status(500).end();});
});
...
async function handleEvent(event){
  ...
  if(text==='情報変更'){sessions[userId]={step:'edit_name'};save('./data/sessions.json',sessions);return reply(event.replyToken,`現在の情報:\n👤${user.name}\n📞${user.phone}\n🚗${user.car}\n\n新しいお名前を入力してください。`);}
  ...
  return reply(event.replyToken,'「予約」で来店予約を開始できます。');
}
